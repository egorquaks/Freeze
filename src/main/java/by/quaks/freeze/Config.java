...
public class Config {
    ...
    public static void setup() {
        ...
        file = new File(Bukkit.getServer().getPluginManager().getPlugin("Freeze").getDataFolder(), name+".yml"); // Загружаем в память файл {name}.yml из папки плагина. Если файла не существует - Java загрузит пустой файл и запомнит что его не существует

        if(!file.exists()){
            try {
                file.createNewFile();
                Bukkit.getServer().getPluginManager().getPlugin("Freeze").saveResource(name+".yml",true);
            } catch (IOException e) {
                e.printStackTrace();
